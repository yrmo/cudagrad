...
int main() {
  ...
  auto e = (a->matmul(b) + c) * d;
  auto f = e->sum();
  f->backward();

  using namespace std;
  for (auto& x : f->data_) {cout<<x<<" ";} // 2794
  for (auto& x : f->size_) {cout<<x<<" ";} // 1
  for (auto& x : a->grad_) {cout<<x<<" ";} // 143 187 143 187
  for (auto& x : b->grad_) {cout<<x<<" ";} // 66 66 88 88
}
